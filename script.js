...
function next(){
 ...
 setTimeout(expertBots,Math.max(100,350/auctionSpeed));
}
...
function place(amount){
  ...
  // A franchise cannot raise its own bid twice consecutively.
  // Once another franchise has raised the bid, this team may bid again.
  if(mode==='ai' && leader===mine.id)return;
  if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()){
...
function setFastForwardSpeed(speed){
  ...
  if(mode==='ai')botTimer=setTimeout(expertBots,Math.max(90,300/auctionSpeed));
}
...
function render(){
 ...
 if(turn){
   if(mode==='ai' && leader===mine.id) turn.innerHTML=`⏳ <b>${leader}</b> is leading • waiting for another franchise to raise`;
   else if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()) turn.innerHTML=`⏱️ BID WINDOW • next bid in ${Math.max(0,(expertBidGapMs()-(performance.now()-lastExpertBidAt))/1000).toFixed(1)}s`;
   else turn.innerHTML=`🎯 <b>${mine.id}</b> can bid now`;
 }
...
function setAuctionNavigationLock(locked){
 ...
 if(back) back.style.display=auctionNavigationLocked?'none':'';
}
...
window.addEventListener('popstate',()=>{
 ...
 if(auctionNavigationLocked || current==='auction'){
   // Do not allow browser Back / Android Back / iOS swipe-back to leave a live auction.
   auctionNavigationLocked=true;
   setAuctionNavigationLock(true);
