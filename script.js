...
/* ============================================================
   BIDORAX IN-GAME DIALOG SYSTEM
   Replaces browser uiAlert()/confirm() popups with styled game UI.
   ============================================================ */
let bidoraxDialogResolve=null;
function ensureGameDialog(){
  if(document.getElementById('gameDialog')) return;
  document.body.insertAdjacentHTML('beforeend', `
    <div id="gameDialog" class="game-dialog" aria-hidden="true">
      <div class="game-dialog-backdrop" onclick="closeGameDialog(false)"></div>
      <div class="game-dialog-card" role="dialog" aria-modal="true" aria-labelledby="gameDialogTitle">
        <div class="game-dialog-icon" id="gameDialogIcon">🏏</div>
        <div class="game-dialog-kicker" id="gameDialogKicker">BIDORAX</div>
        <h3 id="gameDialogTitle">Message</h3>
        <p id="gameDialogMessage"></p>
        <div class="game-dialog-actions" id="gameDialogActions"></div>
      </div>
    </div>`);
}
function openGameDialog(message,{title='BIDORAX',kicker='GAME MESSAGE',icon='🏏',confirm=false,confirmText='CONFIRM',cancelText='CANCEL'}={}){
  ensureGameDialog();
  const modal=document.getElementById('gameDialog');
  ...
  actions.innerHTML=confirm
    ? `<button class="dialog-btn secondary" onclick="closeGameDialog(false)">${cancelText}</button><button class="dialog-btn primary" onclick="closeGameDialog(true)">${confirmText}</button>`
    : `<button class="dialog-btn primary" onclick="closeGameDialog(true)">OK</button>`;
  modal.classList.add('show');modal.setAttribute('aria-hidden','false');
}
function uiAlert(message,title='BIDORAX'){
  openGameDialog(message,{title,kicker:'GAME MESSAGE',icon:'🏏',confirm:false});
}
function uiConfirm(message,{title='END AUCTION',icon='🛑',confirmText='END AUCTION'}={}){
  return new Promise(resolve=>{bidoraxDialogResolve=resolve;openGameDialog(message,{title,kicker:'CONFIRM ACTION',icon,confirm:true,confirmText,cancelText:'CANCEL'});});
}
function closeGameDialog(result=false){
  const modal=document.getElementById('gameDialog');
  if(modal){modal.classList.remove('show');modal.setAttribute('aria-hidden','true');}
  if(bidoraxDialogResolve){const resolve=bidoraxDialogResolve;bidoraxDialogResolve=null;resolve(!!result);}
}

const UNCAPPED_INDIAN_POOL_V5 = [
  {"name":"Ayush Mhatre","role":"Batsman","ovr":84,"category":"uncapped","capped":false,"base":0.2,"matches":6,"runs":245,"wickets":0,"sr":154,"bat":88,"bowl":35,"fin":87},
  {"name":"Angkrish Raghuvanshi","role":"Batsman","ovr":85,"category":"uncapped","capped":false,"base":0.2,"matches":10,"runs":430,"wickets":0,"sr":151,"bat":90,"bowl":38,"fin":88},{"name": "Ayush Doseja", "role": "Batsman", "ovr": 72, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Abhinav Manohar", "role": "Batsman", "ovr": 73, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Aman Khan", "role": "All-rounder", "ovr": 71, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Anuj Rawat", "role": "Wicketkeeper", "ovr": 70, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Arshin Kulkarni", "role": "All-rounder", "ovr": 70, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Ashutosh Sharma", "role": "Batsman", "ovr": 76, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Baba Indrajith", "role": "Wicketkeeper", "ovr": 70, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Bharat Sharma", "role": "Wicketkeeper", "ovr": 71, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Dhruv Jurel", "role": "Wicketkeeper", "ovr": 78, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Harpreet Brar", "role": "All-rounder", "ovr": 74, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Himanshu Rana", "role": "Batsman", "ovr": 70, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Himanshu Sharma", "role": "Bowler", "ovr": 66, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Ishan Porel", "role": "Bowler", "ovr": 68, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Jagadeesan Narayan", "role": "Wicketkeeper", "ovr": 70, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Kumar Kushagra", "role": "Wicketkeeper", "ovr": 72, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Manan Vohra", "role": "Batsman", "ovr": 67, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Mujeeb-style?", "role": "Bowler", "ovr": 65, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Nehal Wadhera", "role": "Batsman", "ovr": 76, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Nishant Sindhu", "role": "All-rounder", "ovr": 69, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Prabhsimran Singh", "role": "Wicketkeeper", "ovr": 74, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Prerak Mankad", "role": "All-rounder", "ovr": 70, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Priyansh Arya", "role": "Batsman", "ovr": 75, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Raj Angad Bawa", "role": "All-rounder", "ovr": 68, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Rajat Dey", "role": "Batsman", "ovr": 66, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Ramandeep Singh", "role": "All-rounder", "ovr": 73, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Rasikh Salam", "role": "Bowler", "ovr": 72, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Suyash Sharma", "role": "Bowler", "ovr": 73, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Tanush Kotian", "role": "All-rounder", "ovr": 74, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Vaibhav Arora", "role": "Bowler", "ovr": 72, "category": "uncapped", "capped": false, "base": 0.2}, {"name": "Vijaykumar Vyshak", "role": "Bowler", "ovr": 71, "category": "uncapped", "capped": false, "base": 0.2}];

// BIDORAX V5 POOL STRUCTURE
// Each pool is completed fully before the next pool starts.
// Every auction pool is expanded by 5 players in V10.

const BIDORAX_MARQUEE_EXTRAS = [
  {name:'Mitchell Marsh',role:'AR',ovr:91,bat:88,bowl:78,fin:92,matches:55,runs:1500,wickets:40,sr:145,base:1.25},
  {name:'Cameron Green',role:'AR',ovr:91,bat:87,bowl:82,fin:88,matches:35,runs:900,wickets:25,sr:142,base:1},
...
const BIDORAX_ROLE_EXTRAS = [
  ...
  {name:'Marcus Stoinis',role:'AR',ovr:87,bat:88,bowl:76,fin:91,matches:65,runs:1700,wickets:35,sr:145,base:0.75},
  {name:'Jason Holder',role:'AR',ovr:86,bat:78,bowl:86,fin:84,matches:60,runs:1000,wickets:55,sr:135,base:0.75},
  {name:'Sikandar Raza',role:'AR',ovr:86,bat:84,bowl:78,fin:87,matches:50,runs:1200,wickets:25,sr:140,base:0.6},
  {name:'Azmatullah Omarzai',role:'AR',ovr:85,bat:80,bowl:83,fin:86,matches:35,runs:650,wickets:30,sr:138,base:0.5},
  {name:'Mohammad Nabi',role:'AR',ovr:84,bat:76,bowl:84,fin:82,matches:70,runs:1300,wickets:55,sr:135,base:0.5}
];
...
const DATA=[
['Virat Kohli','BAT',94,94,20,95,252,8600,0,132,2],['Rohit Sharma','BAT',94,93,20,88,264,7000,0,130,2],['Shubman Gill','BAT',91,92,20,87,104,3200,0,136,1.5],['Suryakumar Yadav','BAT',95,96,20,96,150,3590,0,168,1.5],['Yashasvi Jaiswal','BAT',91,92,20,91,55,1700,0,150,1],['Ruturaj Gaikwad','BAT',90,90,20,84,70,2380,0,136,1],['Shreyas Iyer','BAT',90,89,20,84,115,3100,0,128,1],['Rinku Singh','BAT',86,87,20,95,45,900,0,142,.75],['Rishabh Pant','WK',92,91,15,94,111,3300,0,148,1.5],['Sanju Samson','WK',90,89,15,91,160,4410,0,145,1.5],['KL Rahul','WK',91,90,15,88,132,4680,0,135,1.5],['Nicholas Pooran','WK',91,90,15,96,90,1900,0,157,1.5],['Heinrich Klaasen','WK',92,91,15,96,85,1900,0,171,1.5],['Jasprit Bumrah','BOWL',95,30,95,65,140,0,165,0,2],['Rashid Khan','BOWL',95,45,96,60,120,500,150,0,2],['Pat Cummins','BOWL',92,55,91,66,58,500,105,0,1.5],['Mohammed Shami','BOWL',91,30,94,62,110,0,127,0,1.5],['Mohammed Siraj','BOWL',90,25,89,60,93,0,97,0,1],['Arshdeep Singh','BOWL',87,20,89,60,65,0,80,0,1],['Kuldeep Yadav','BOWL',91,40,91,58,75,300,90,0,1],['Trent Boult','BOWL',90,25,93,61,90,0,105,0,1],['Kagiso Rabada','BOWL',90,25,92,63,75,0,117,0,1],['Hardik Pandya','AR',93,88,86,93,145,2300,73,145,1.5],['Ravindra Jadeja','AR',93,85,89,86,240,2900,160,130,1.5],['Andre Russell','AR',91,87,82,98,115,2200,96,175,1.5],['Sunil Narine','AR',92,80,91,82,180,1600,172,168,1.5],['Glenn Maxwell','AR',90,89,74,95,130,2800,40,158,1.25],['Axar Patel','AR',90,78,86,76,150,1400,123,132,1],['Abhishek Sharma','AR',91,90,72,92,65,1500,25,178,.75],['Washington Sundar','AR',90,76,84,70,70,900,38,130,.5],['Jos Buttler','WK',94,93,10,97,110,3800,0,150,2],['Travis Head','BAT',93,95,10,96,50,1800,0,172,1.5],['Faf du Plessis','BAT',90,88,10,82,154,4570,0,136,1],['David Warner','BAT',88,89,10,87,184,6560,0,140,1],['Mitchell Starc','BOWL',91,22,94,63,60,0,70,0,1.5],['Yuzvendra Chahal','BOWL',88,25,91,55,160,0,205,0,1],['Bhuvneshwar Kumar','BOWL',86,25,87,55,180,0,181,0,1],['Avesh Khan','BOWL',82,18,84,55,75,0,85,0,.5],['T Natarajan','BOWL',84,18,86,58,65,0,90,0,.75],['Deepak Chahar','BOWL',82,35,82,62,80,0,76,0,.5],['Shivam Dube','AR',85,87,62,91,70,1600,18,151,.75],['Riyan Parag','AR',84,86,55,90,60,1300,10,155,.75],['Tilak Varma','BAT',86,88,20,87,50,1300,0,145,.75],['Ishan Kishan','WK',85,84,10,88,105,2800,0,135,.75],['Devon Conway','WK',88,89,10,85,50,1600,0,139,1],['Tim David','BAT',84,83,10,95,40,800,0,170,.5],['Rajat Patidar','BAT',84,86,10,86,45,1200,0,158,.75],['Venkatesh Iyer','AR',83,85,62,87,60,1400,5,145,.5],['Nitish Rana','AR',82,84,55,83,100,2600,10,135,.5],['Harshal Patel','BOWL',83,25,82,70,130,900,130,132,.75],['Varun Chakravarthy','BOWL',87,25,91,58,90,100,100,125,1],['Mukesh Kumar','BOWL',79,20,82,55,40,0,45,0,.5],['Mayank Yadav','BOWL',88,20,94,60,20,0,25,0,1],['Matheesha Pathirana','BOWL',87,15,93,61,35,0,55,0,1],['Noor Ahmad','BOWL',86,30,90,58,35,100,45,0,.75],['Jitesh Sharma','WK',81,80,10,88,40,700,0,147,.5],['Ayush Badoni','BAT',79,81,15,84,45,700,0,140,.5]
];

const CAPPED_BATTERS = [
 ['Ruturaj Gaikwad','BAT',90,90,20,84,70,2380,0,136,1],['Shreyas Iyer','BAT',90,89,20,84,115,3100,0,128,1],['Faf du Plessis','BAT',90,88,10,82,154,4570,0,136,1],['Sai Sudharsan','BAT',90,92,20,88,40,1600,0,165,1],['Quinton de Kock','BAT',89,90,10,90,110,3150,0,150,1],
 ['Sai Sudharsan','BAT',90,92,20,88,40,1600,0,165,1],['Devdutt Padikkal','BAT',82,83,20,78,70,1800,0,126,.5],['Mayank Agarwal','BAT',84,85,20,80,123,2600,0,128,.75],['Mandeep Singh','BAT',79,80,15,78,170,2700,0,125,.5],['Manish Pandey','BAT',80,81,15,80,171,3800,0,121,.5],['Ajinkya Rahane','BAT',82,83,15,78,185,4650,0,124,.5],['Karun Nair','BAT',79,81,10,80,76,1490,0,127,.5],['Kedar Jadhav','BAT',78,78,20,84,95,1200,0,140,.4],['Saurabh Tiwary','BAT',76,77,15,76,115,1490,0,120,.3],['Abhishek Porel','BAT',80,82,15,80,30,650,0,140,.5],['Shahrukh Khan','BAT',82,83,20,91,45,750,0,170,.6],['Rahmanullah Gurbaz','BAT',84,84,10,90,45,1000,0,155,.75],['Jason Roy','BAT',83,84,10,88,68,1500,0,140,.75],['Evin Lewis','BAT',82,83,10,91,35,900,0,165,.75],['Quinton de Kock','BAT',89,90,10,90,110,3150,0,150,1],['Aiden Markram','BAT',86,86,40,84,45,1050,20,140,1],['Kane Williamson','BAT',88,89,20,78,75,2200,0,130,1],['Dawid Malan','BAT',84,85,10,82,50,1100,0,135,.75],['Jonny Bairstow','BAT',87,88,10,91,50,1600,0,145,1],['Steve Smith','BAT',86,87,20,80,40,1000,0,125,.75],['Dinesh Karthik','BAT',81,80,15,88,257,4842,0,135,.5],['Robin Uthappa','BAT',80,82,10,82,205,4950,0,130,.4],['Murali Vijay','BAT',78,79,10,75,106,2619,0,120,.3],['Aaron Finch','BAT',82,83,10,85,92,2091,0,135,.5],['Colin Munro','BAT',80,81,10,87,13,177,0,160,.4],['Martin Guptill','BAT',79,80,10,84,13,270,0,135,.35],['Chris Lynn','BAT',80,81,10,88,42,1329,0,140,.5],['Alex Hales','BAT',83,84,10,90,23,577,0,150,.5],['David Miller','BAT',86,88,10,92,127,2924,0,138,.75],['Shimron Hetmyer','BAT',83,84,10,90,74,1320,0,155,.6],['Rilee Rossouw','BAT',82,83,10,89,19,428,0,150,.5],['Martin Guptill','BAT',79,80,10,84,13,270,0,135,.35]
];
const CAPPED_BOWLERS = [
 ['Mohammed Siraj','BOWL',90,25,89,60,93,0,97,0,1],['Kuldeep Yadav','BOWL',91,40,91,58,75,300,90,0,1],['Kagiso Rabada','BOWL',90,25,92,63,75,0,117,0,1],['Trent Boult','BOWL',90,25,93,61,90,0,105,0,1],['Josh Hazlewood','BOWL',89,20,92,60,15,0,12,0,1],['Lasith Malinga','BOWL',89,10,91,60,122,0,170,0,.8],
 ['Basil Thampi','BOWL',76,20,79,55,35,0,28,0,.3],['Sandeep Sharma','BOWL',82,20,84,58,130,0,145,0,.5],['Jaydev Unadkat','BOWL',80,20,82,55,105,0,105,0,.4],['Ishant Sharma','BOWL',78,18,80,55,110,0,92,0,.3],['Umesh Yadav','BOWL',82,20,85,58,148,0,145,0,.5],['Varun Aaron','BOWL',75,18,78,55,52,0,44,0,.3],['Dhawal Kulkarni','BOWL',76,18,79,55,92,0,86,0,.3],['Mohit Sharma','BOWL',80,20,83,60,110,0,132,0,.4],['Piyush Chawla','BOWL',82,25,84,55,192,0,192,0,.5],['Amit Mishra','BOWL',80,20,84,52,162,0,173,0,.4],['R Sai Kishore','BOWL',84,25,88,58,50,100,55,0,.5],['Ravi Bishnoi','BOWL',86,25,89,58,60,120,75,0,.75],['Mohsin Khan','BOWL',81,18,85,60,25,0,25,0,.4],['Khaleel Ahmed','BOWL',83,20,86,60,60,0,70,0,.5],['Navdeep Saini','BOWL',79,18,83,58,45,0,40,0,.4],['Chetan Sakariya','BOWL',78,18,82,58,35,0,40,0,.35],['Tushar Deshpande','BOWL',80,18,83,59,40,0,50,0,.45],['Akash Deep','BOWL',82,20,85,58,30,0,35,0,.5],['Yash Dayal','BOWL',84,18,87,60,40,0,50,0,.6],['Umran Malik','BOWL',81,15,88,58,25,0,29,0,.5],['Shreyas Gopal','BOWL',77,30,82,55,50,250,48,0,.3],['Kartik Tyagi','BOWL',76,15,81,55,35,0,30,0,.3],['Ankit Rajpoot','BOWL',75,18,79,55,50,0,45,0,.3],['Varun Chakravarthy','BOWL',87,25,91,58,90,100,100,125,1],['Mohammed Shami','BOWL',91,30,94,62,110,0,127,0,1.5],['Lockie Ferguson','BOWL',82,18,88,58,67,0,61,0,.6],['Josh Hazlewood','BOWL',89,20,92,60,15,0,12,0,1],['Adam Zampa','BOWL',85,25,88,57,8,0,12,0,.6],['Tabraiz Shamsi','BOWL',81,20,86,55,18,0,23,0,.45],['Dale Steyn','BOWL',82,15,89,55,95,0,97,0,.5],['Lasith Malinga','BOWL',89,10,91,60,122,0,170,0,.8],['Chris Woakes','BOWL',82,35,84,65,21,0,30,0,.5]
...
let auctionSpeed=1;
let auctionNavigationLocked=false;
try{mpClientId=localStorage.getItem('hh_mp_client_id')||crypto.randomUUID();localStorage.setItem('hh_mp_client_id',mpClientId)}catch(e){mpClientId='hh-'+Date.now()+'-'+Math.random().toString(36).slice(2)}
function mpInit(){
...
function mpJoinChannel(code,isHost,options={}){
 ...
 return new Promise((resolve,reject)=>{
   ...
   mpChannel.on('broadcast',{event:'auction'},({payload})=>{
     ...
     if(payload.type==='join_reject'&&payload.to===mpClientId){uiAlert(payload.reason||'Unable to join room.');return}
     ...
     if(payload.type==='swap_request'&&roomCreator){
       ...
       if(targetMember.clientId===mpClientId){
         ...
         renderManagement();
         uiAlert(`📨 SWAP REQUEST: ${from.id} offers ${fromPlayer.name} for your ${toPlayer.name}.`);
       }else{
     ...
     if(payload.type==='swap_offer'&&payload.toClient===mpClientId){
       ...
       renderManagement();
       uiAlert(`📨 SWAP REQUEST: ${payload.fromTeam} offers ${payload.offer} for your ${payload.request}.`);
       return;
     }
     if(payload.type==='swap_result'){
       if(payload.ok) uiAlert(`🤝 SWAP ACCEPTED: ${payload.offer} ↔ ${payload.request}.`);
       else uiAlert(`❌ SWAP REJECTED: ${payload.reason||'The target team rejected the deal.'}`);
...
const HH_RANKS=[
 ...
 {name:'Gold',icon:'🥇',min:250,max:499,color:'#f5c451'},
 {name:'Platinum',icon:'💠',min:500,max:849,color:'#72d7e8'},
 {name:'Diamond',icon:'💎',min:850,max:1299,color:'#8aa7ff'},
 {name:'Master',icon:'🔥',min:1300,max:1899,color:'#d58bff'},
 {name:'Grandmaster',icon:'👑',min:1900,max:999999,color:'#ff6f91'}
];
function hhRankData(){
...
function hhRecordRivalPosition(position){
 const r=hhRankData();r.auctions++;
 const rivalMode=mode==='rival';
 if(position===1){r.wins++;r.rating+=rivalMode?40:30;}
 else if(position===2){r.rating+=rivalMode?30:20;}
 else if(position===3){r.rating+=rivalMode?15:5;}
 else {r.losses++;r.rating=Math.max(0,r.rating-10);}
 hhSaveRank(r);hhRenderRank();
}
function hhRivalRankPanel(){
 const r=hhRankData(),rank=hhRankInfo(r);
 return `<div class="panel"><h3>🆚 YOUR RIVAL PROFILE</h3><div class="rank-profile"><strong>${rank.icon} ${rank.name}</strong><b>${r.rating} Rating</b><span>W ${r.wins} • L ${r.losses} • D ${r.draws}</span></div><p class="muted">Climb through progressively larger rank thresholds: Silver 100, Gold 250, Platinum 500, Diamond 850, Master 1300, Grandmaster 1900. Rating is saved on this browser.</p></div>`;
}
...
async function findOnlineRival(){
 ...
 if(onlineUsername==='Player')return uiAlert('Enter your username/team name first.');
 if(!mpClient&&!mpInit())return uiAlert('Online Rival is not configured. Add your Supabase URL and anon key in supabase-config.js.');
...
async function startOnlineRivalMatch(match){
 ...
 show('auction');updatePoolUI();renderAuctionControls();lockAuctionHistory();next();
 if(onlineLobbyChannel)try{mpClient.removeChannel(onlineLobbyChannel)}catch(e){}
...
function choose(id){if(mode==='rival')return; if(mode==='multi'&&mpMembers.some(m=>m.teamId===id))return uiAlert('That team is already taken in this room.');chosen=id;document.querySelectorAll('.team').forEach(x=>x.classList.remove('sel'));document.getElementById('t-'+id).classList.add('sel');if((mode==='multi'||mode==='rival'))refreshLobby()}
async function createRoom(){
 if(!chosen)return uiAlert('Select your team first.');
 if(!mpClient&&!mpInit())return uiAlert('Multiplayer is not configured yet. Open supabase-config.js and add your Supabase URL and anon key.');
 roomCreator=true;mpMembers=[{clientId:mpClientId,teamId:chosen}];const c=String(Math.floor(100000+Math.random()*900000));
 try{await mpJoinChannel(c,true);mpLobby();log(`🏟️ ROOM ${c} CREATED • ${chosen} joined the room.`)}catch(e){roomCreator=false;uiAlert('Could not create room: '+e.message)}
}
async function joinRoom(){
 const c=(document.getElementById('code')?.value||'').trim();
 if(!chosen)return uiAlert('Select your team first.');
 if(!/^\d{6}$/.test(c))return uiAlert('Enter a valid 6-digit room code.');
 if(!mpClient&&!mpInit())return uiAlert('Multiplayer is not configured yet. Open supabase-config.js and add your Supabase URL and anon key.');
 roomCreator=false;mpMembers=[];
 try{await mpJoinChannel(c,false);setTimeout(()=>{if(!mpMembers.length)mpChannel.send({type:'broadcast',event:'auction',payload:{type:'join_request',from:mpClientId,member:{clientId:mpClientId,teamId:chosen}}})},350);uiAlert('Connected to room '+c+'. Waiting for the room creator to start the auction.');}catch(e){uiAlert('Could not join room. Check the code and make sure the host is online.')}
}
function start(){
 if(!chosen)return uiAlert('Select a team.');
 if((mode==='multi'||mode==='rival')&&!roomCreator)return uiAlert('Only the room creator starts the multiplayer auction.');
 ...
 show('auction'); updatePoolUI(); renderAuctionControls(); lockAuctionHistory(); next(); mpPublish();
}
...
function buildAuctionPools(){
  ...
  for(const p of all){ if(!seen.has(p.name)){seen.add(p.name);unique.push(p)} }
  ...
  // MARQUEE contains exactly 20 elite players, all rated above 90.
  // Selected 89-90 players are deliberately kept in role pools so the marquee remains exclusive.
  const role90PlusProtected=new Set(['Ruturaj Gaikwad','Shreyas Iyer','Faf du Plessis','Mohammed Siraj','Kuldeep Yadav','Kagiso Rabada','Axar Patel','Abhishek Sharma','Washington Sundar','Sanju Samson','KL Rahul','Nicholas Pooran','Sai Sudharsan','Quinton de Kock','Trent Boult','Josh Hazlewood','Lasith Malinga']);
  const marquee=unique.filter(p=>p.ovr>90 && p.name!=='Deepak Chahar' && !role90PlusProtected.has(p.name))
  ...
  // Each non-marquee capped pool intentionally contains at least 3 players rated 90+ OVR.
...
function next(){
 ...
 round++; bid=cur.base;leader='';time=EXPERT_AUCTION_START_TIME;lastExpertBidAt=0;lastOutcome=null;auctionSpeed=auctionSpeed||1;render();mpPublish();
 ...
 setTimeout(expertBots,Math.max(1500,1500/auctionSpeed));
}
...
function expertBots(){
  ...
  if((mode==='multi'||mode==='rival')&&!roomCreator)return;
  if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()){
    botTimer=setTimeout(expertBots,Math.max(25,expertBidGapMs()-(performance.now()-lastExpertBidAt)));
    return;
  }
  ...
  // Smart, selective AI: prefer value and squad needs over early aggression.
  // Give human players a real opening, then increase pressure only when the player fits a strategic need.
  const needNow=botNeed(best.team,cur);
  const openingBid=(leader==='');
  let chance=.40+urgency*.12+(cur.ovr>=90?.07:0)+(competitiveGap<8?.06:0)+(needNow>0?.07:0);
  if(openingBid) chance-=.14;
  if(needNow===0) chance-=.16;
  if(best.team.squad.length>=11) chance-=.05;
  chance=Math.max(.20,Math.min(.86,chance));
  ...
  botTimer=setTimeout(expertBots,Math.max(25,expertBidGapMs()));
}
...
function place(amount){
  ...
  if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()){
    // Keep the normal BID buttons visible; simply wait for the 1.5s gap.
    return;
  }
  if(mine.squad.length>=maxSquadSize()||mine.purse<bid+amountNum)return uiAlert('You cannot afford this bid or your squad is full.');
  if((mode==='multi'||mode==='rival')&&!roomCreator){
    if(!mpChannel||!mpConnected)return uiAlert('You are not connected to the auction room.');
    mpChannel.send({type:'broadcast',event:'auction',payload:{type:'bid_request',teamId:mine.id,amount:amountNum,from:mpClientId}}).catch(()=>uiAlert('Bid could not be sent.'));
    return;
  }
  bid=+(bid+amountNum).toFixed(2);leader=mine.id;time=EXPERT_AUCTION_START_TIME;lastExpertBidAt=performance.now();
  log(`🔥 ${mine.id} bids ₹${bid.toFixed(2)} Cr`);render();mpPublish();
  clearTimeout(botTimer);botTimer=setTimeout(expertBots,Math.max(25,expertBidGapMs()));
}
...
function simulateExpertPlayer(p){
  ...
  let guard=0;
  while(guard++<80){
    const candidates=teams.filter(t=>t.id!==leader&&t.squad.length<maxSquadSize()&&t.purse>=bid+STEP&&t.purse>0)
    ...
    // Same selective strategy for automatic end-of-auction simulation.
    const needNow=botNeed(best.team,p);
    let chance=.48+urgency*.10+(p.ovr>=90?.07:0)+(gap<8?.06:0)+(needNow>0?.08:0);
    if(needNow===0)chance-=.15;
    chance=Math.max(.30,Math.min(.90,chance));
...
function botUrgency(t,p){return Math.min(6,roleDeficit(t,p.role)+Math.max(0,11-t.squad.length)*.35)}
...
function botNeed(t,p){
 const direct=roleDeficit(t,p.role);let need=direct;if(teamBalanceScore(t)<82&&direct>0)need+=1;if(players.length<20&&direct>0)need+=1;if(t.squad.length>=Math.max(12,maxSquadSize()-6)&&direct===0)need=Math.max(0,need-1);return need;
}
...
function setFastForwardSpeed(speed){
  ...
  if(![1,2,4,8].includes(selected))return;
  auctionSpeed=selected;
  clearInterval(timer);clearTimeout(botTimer);
  log(auctionSpeed===1?`▶️ NORMAL SPEED • ${cur.name} auction speed restored.`:`⏩ ${auctionSpeed}× FAST FORWARD • ${cur.name} auction speed increased. No player is skipped or forced unsold.`);
  ...
  if(mode==='ai')botTimer=setTimeout(expertBots,Math.max(1500,1500/auctionSpeed));
}
...
async function endAuction(){
  if((mode==='multi'||mode==='rival')&&!roomCreator)return uiAlert('Only the room creator can end the multiplayer auction.');
  if(mode==='ai'){
    if(!(await uiConfirm('End the live auction? Expert AI will automatically complete the remaining players using each team\'s remaining purse, squad needs and team balance.',{title:'END LIVE AUCTION',icon:'🤖',confirmText:'END AUCTION'})))return;
    log('🛑 END AUCTION CONFIRMED • Expert AI is completing the remaining auction.');
    autoCompleteExpertAuction();
    return;
  }
  if(!(await uiConfirm('End the live auction for everyone? The auction will stop and move to the management stage.',{title:'END AUCTION FOR EVERYONE',icon:'🛑',confirmText:'END FOR EVERYONE'})))return;
  auctionEnded=true;
  clearInterval(timer);
...
function swapPlayer(myName,targetTeamId,targetName){
 ...
 if(t===mine)return uiAlert('Choose another team.');
 if(mode==='ai'){
   if(!swapAcceptableForAI(a,b,t)){
     ...
     return uiAlert(`🤖 ${t.id} rejects the deal. OVR gap: ${gap}. ${need}. AI accepts only within ±4 OVR and when it needs that role.`);
   }
 ...
 if(!mpChannel||!mpConnected)return uiAlert('You are not connected to the auction room.');
 mpChannel.send({type:'broadcast',event:'auction',payload:{type:'swap_request',fromTeam:mine.id,toTeam:t.id,offer:a.name,request:b.name,from:mpClientId}}).catch(()=>uiAlert('Swap proposal could not be sent.'));
 renderManagement();
 uiAlert(`📨 Swap proposal sent to ${t.id}. Only the ${t.id} owner can accept or reject it.`);
}
function respondToSwap(accept){
 if(!pendingSwap)return;
 // Only the target team is allowed to respond. The proposing team has no pendingSwap.
 if(pendingSwap.to!==mine.id)return uiAlert('Only the team receiving the proposal can accept or reject this deal.');
...
function openManagement(){clearInterval(timer);clearTimeout(botTimer);unlockAuctionNavigation();show('manage');renderManagement()}
...
function doSwap(){const my=decodeURIComponent(document.getElementById('mySwap').value||'');const target=(document.getElementById('targetSwap').value||'').split('|');if(!my||target.length<2)return uiAlert('Select both players.');swapPlayer(my,target[0],decodeURIComponent(target[1]))}
...
function render(){
 ...
 const blocked=false;
 document.getElementById('bids').innerHTML=[.25,.5,1,2,5].map(x=>`<button onclick="place(${x})">+₹${x} Cr</button>`).join('');
 const turn=document.getElementById('turnHint');
 if(turn){
   if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()){
     turn.innerHTML=`🎯 BIDDING`;
   }else{
     turn.innerHTML=`🎯 BIDDING`;
   }
 }
...
function renderClock(){document.getElementById('clock').textContent='00:'+String(Math.max(0,time)).padStart(2,'0')}
function score(t){return teamBalanceScore(t)}
function finish(){unlockAuctionNavigation();clearInterval(timer);const activeTeams=participatingTeams();activeTeams.forEach(t=>t.score=score(t));activeTeams.sort((a,b)=>b.score-a.score);
...
let bidoraxHistoryNav=false;
const BIDORAX_PARENT={home:null,setup:'home',auction:'setup',manage:'auction',results:'manage'};
function setAuctionNavigationLock(locked){
 auctionNavigationLocked=!!locked;
 const back=document.querySelector('.auction-back');
 if(back) back.style.display='';
}
function lockAuctionHistory(){
 setAuctionNavigationLock(true);
 // Keep the current auction state as the active browser entry. Any Android/iOS
 // Back, Chrome Back, or swipe-back attempt is intercepted while the auction is live.
 const state={bidorax:true,screen:'auction',auctionLocked:true};
 history.replaceState(state,'','#auction');
}
function unlockAuctionNavigation(){setAuctionNavigationLock(false);}

function syncBidoraxHistory(id,replace=false){
 const state={bidorax:true,screen:id};
 const url=id==='home'?'#home':'#'+id;
...
function show(id,options={}){
 ...
 if(!options.fromHistory && !bidoraxHistoryNav) syncBidoraxHistory(id);
 window.scrollTo({top:0,left:0,behavior:"auto"});
}
async function confirmExitAuction(){
 const leave=await uiConfirm('Are you sure you want to exit the live auction? Your current auction progress will be left.','EXIT AUCTION',{icon:'🚪',confirmText:'EXIT AUCTION'});
 if(!leave){
   history.replaceState({bidorax:true,screen:'auction',auctionLocked:true},'', '#auction');
   setAuctionNavigationLock(true);
   return false;
 }
 unlockAuctionNavigation();
 clearInterval(timer);
 clearTimeout(botTimer);
 bidoraxHistoryNav=true;
 try{
   history.replaceState({bidorax:true,screen:'setup'},'', '#setup');
   show('setup',{fromHistory:true});
 } finally { bidoraxHistoryNav=false; }
 return true;
}
function navigateBack(){
 const current=document.querySelector('.screen.active')?.id||'home';
 if(current==='home') return;
 if(current==='auction' || auctionNavigationLocked){ confirmExitAuction(); return; }
 if(history.state?.bidorax){ history.back(); return; }
 const parent=BIDORAX_PARENT[current]||'home';
 show(parent);
}
window.addEventListener('popstate',()=>{
 const current=document.querySelector('.screen.active')?.id||'home';
 if(auctionNavigationLocked || current==='auction'){
   history.pushState({bidorax:true,screen:'auction',auctionLocked:true},'', '#auction');
   setAuctionNavigationLock(true);
   confirmExitAuction();
   return;
 }
 let target=history.state?.bidorax?history.state.screen:null;
 if(!target){ target=BIDORAX_PARENT[current]||'home'; }
 bidoraxHistoryNav=true;
 try{ show(target,{fromHistory:true}); } finally { bidoraxHistoryNav=false; }
});
function home(){unlockAuctionNavigation();clearInterval(timer);clearTimeout(botTimer);
 if(document.querySelector(".screen.active")?.id==='home')return;
 show('home');
}
make();
if(!history.state?.bidorax) syncBidoraxHistory('home',true);
...
function renderAuctionControls(){
  ...
  if(select){
    ...
    select.value=String([1,2,4,8].includes(auctionSpeed)?auctionSpeed:1);
