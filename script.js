...
function place(amount){
  ...
  if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()){
    const wait=(expertBidGapMs()-(performance.now()-lastExpertBidAt))/1000;
    return uiAlert(`Another team just bid. You can bid again in ${Math.max(0.1,wait).toFixed(1)} seconds.`, 'BID WINDOW');
  }
...
}function home(){clearInterval(timer);clearTimeout(botTimer);show('home')}make();
