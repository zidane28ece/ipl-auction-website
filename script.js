...
const BIDORAX_MARQUEE_EXTRAS = [
  {name:'Babar Azam',role:'BAT',ovr:91,bat:94,bowl:30,fin:90,matches:70,runs:2800,wickets:0,sr:145,base:1.5},
...
const AUCTION_POOLS = [
  { key:"marquee", title:"MARQUEE", icon:"🔥", count:35 },
...
const DATA=[
['Virat Kohli','BAT',96,95,20,95,252,8600,0,132,2],['Rohit Sharma','BAT',94,93,20,88,264,7000,0,130,2],['Shubman Gill','BAT',93,94,20,87,104,3200,0,136,1.5],['Suryakumar Yadav','BAT',95,96,20,96,150,3590,0,168,1.5],['Yashasvi Jaiswal','BAT',91,92,20,91,55,1700,0,150,1],['Ruturaj Gaikwad','BAT',89,91,20,84,70,2380,0,136,1],['Shreyas Iyer','BAT',88,89,20,84,115,3100,0,128,1],['Rinku Singh','BAT',86,87,20,95,45,900,0,142,.75],['Rishabh Pant','WK',92,91,15,94,111,3300,0,148,1.5],['Sanju Samson','WK',90,89,15,91,160,4410,0,145,1.5],['KL Rahul','WK',91,90,15,88,132,4680,0,135,1.5],['Nicholas Pooran','WK',91,90,15,96,90,1900,0,157,1.5],['Heinrich Klaasen','WK',92,91,15,96,85,1900,0,171,1.5],['Jasprit Bumrah','BOWL',97,30,97,65,140,0,165,0,2],['Rashid Khan','BOWL',95,45,96,60,120,500,150,0,2],['Pat Cummins','BOWL',92,55,91,66,58,500,105,0,1.5],['Mohammed Shami','BOWL',91,30,94,62,110,0,127,0,1.5],['Mohammed Siraj','BOWL',88,25,89,60,93,0,97,0,1],['Arshdeep Singh','BOWL',87,20,89,60,65,0,80,0,1],['Kuldeep Yadav','BOWL',89,40,91,58,75,300,90,0,1],['Trent Boult','BOWL',90,25,93,61,90,0,105,0,1],['Kagiso Rabada','BOWL',89,25,92,63,75,0,117,0,1],['Hardik Pandya','AR',93,88,86,93,145,2300,73,145,1.5],['Ravindra Jadeja','AR',93,85,89,86,240,2900,160,130,1.5],['Andre Russell','AR',91,87,82,98,115,2200,96,175,1.5],['Sunil Narine','AR',92,80,91,82,180,1600,172,168,1.5],['Glenn Maxwell','AR',90,89,74,95,130,2800,40,158,1.25],['Axar Patel','AR',88,78,86,76,150,1400,123,132,1],['Abhishek Sharma','AR',88,90,72,92,65,1500,25,178,.75],['Washington Sundar','AR',84,76,84,70,70,900,38,130,.5],['Jos Buttler','WK',94,93,10,97,110,3800,0,150,2],['Travis Head','BAT',93,95,10,96,50,1800,0,172,1.5],['Faf du Plessis','BAT',87,88,10,82,154,4570,0,136,1],['David Warner','BAT',88,89,10,87,184,6560,0,140,1],['Mitchell Starc','BOWL',91,22,94,63,60,0,70,0,1.5],['Yuzvendra Chahal','BOWL',88,25,91,55,160,0,205,0,1],['Bhuvneshwar Kumar','BOWL',86,25,87,55,180,0,181,0,1],['Avesh Khan','BOWL',82,18,84,55,75,0,85,0,.5],['T Natarajan','BOWL',84,18,86,58,65,0,90,0,.75],['Deepak Chahar','BOWL',82,35,82,62,80,0,76,0,.5],['Shivam Dube','AR',85,87,62,91,70,1600,18,151,.75],['Riyan Parag','AR',84,86,55,90,60,1300,10,155,.75],['Tilak Varma','BAT',86,88,20,87,50,1300,0,145,.75],['Ishan Kishan','WK',85,84,10,88,105,2800,0,135,.75],['Devon Conway','WK',88,89,10,85,50,1600,0,139,1],['Tim David','BAT',84,83,10,95,40,800,0,170,.5],['Rajat Patidar','BAT',84,86,10,86,45,1200,0,158,.75],['Venkatesh Iyer','AR',83,85,62,87,60,1400,5,145,.5],['Nitish Rana','AR',82,84,55,83,100,2600,10,135,.5],['Harshal Patel','BOWL',83,25,82,70,130,900,130,132,.75],['Varun Chakravarthy','BOWL',87,25,91,58,90,100,100,125,1],['Mukesh Kumar','BOWL',79,20,82,55,40,0,45,0,.5],['Mayank Yadav','BOWL',88,20,94,60,20,0,25,0,1],['Matheesha Pathirana','BOWL',87,15,93,61,35,0,55,0,1],['Noor Ahmad','BOWL',86,30,90,58,35,100,45,0,.75],['Jitesh Sharma','WK',81,80,10,88,40,700,0,147,.5],['Ayush Badoni','BAT',79,81,15,84,45,700,0,140,.5]
];
...
function buildAuctionPools(){
  ...
  // MARQUEE is reserved first: every marquee player must be rated above 88.
  // This explicitly keeps Jasprit Bumrah in MARQUEE and prevents Deepak Chahar (82) from entering it.
  const marquee=unique.filter(p=>p.ovr>88 && p.name!=='Deepak Chahar')
...
function next(){
 ...
 setTimeout(expertBots,mode==='ai'?EXPERT_BID_GAP_MS:Math.max(100,350/auctionSpeed));
}
...
function expertBots(){
  ...
  if(mode==='ai'){
    ...
    if(elapsed<EXPERT_BID_GAP_MS){
      clearTimeout(botTimer);
      botTimer=setTimeout(expertBots,Math.max(40,EXPERT_BID_GAP_MS-elapsed));
      return;
    }
  }
  ...
  if(Math.random()>chance){
    if(mode==='ai')botTimer=setTimeout(expertBots,EXPERT_BID_GAP_MS);
    return;
  }
  ...
  botTimer=setTimeout(expertBots,mode==='ai'?EXPERT_BID_GAP_MS:Math.max(80,(520-Math.round(cur.ovr*2))/auctionSpeed));
}
...
function place(amount){
  ...
  clearTimeout(botTimer);botTimer=setTimeout(expertBots,mode==='ai'?EXPERT_BID_GAP_MS:Math.max(80,180/auctionSpeed));
}
...
function setFastForwardSpeed(speed){
  ...
  if(mode==='ai')botTimer=setTimeout(expertBots,Math.max(EXPERT_BID_GAP_MS,300/auctionSpeed));
}
...
function renderClock(){const s=Math.max(0,time);const whole=Math.floor(s);const tenth=Math.floor((s-whole)*10+1e-6);document.getElementById('clock').textContent='00:'+String(whole).padStart(2,'0')+'.'+tenth}
