...
function place(amount){
  ...
  if(mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs()){
    // Keep the normal bid buttons visible and usable. A click during the
    // short inter-bid window simply waits until the 1.5s gap has elapsed;
    // no popup interrupts the auction.
    return;
  }
