...
function next(){
 ...
 round++; bid=cur.base;leader='';time=10;lastOutcome=null;auctionSpeed=auctionSpeed||1;render();mpPublish();
...
function expertBots(){
  ...
  time=Math.min(10,Math.max(2,time+1));render();
  if((mode==='multi'||mode==='rival'))mpPublish();
  botTimer=setTimeout(expertBots,Math.max(80,(520-Math.round(cur.ovr*2))/auctionSpeed));
}
...
function place(amount){
  ...
  bid=+(bid+amountNum).toFixed(2);leader=mine.id;time=Math.min(10,Math.max(2,time+2));
  log(`🔥 ${mine.id} bids ₹${bid.toFixed(2)} Cr`);render();mpPublish();
  clearTimeout(botTimer);botTimer=setTimeout(expertBots,Math.max(80,180/auctionSpeed));
}
...
function setFastForwardSpeed(speed){
  ...
  if(![1,2,4,10].includes(selected))return;
...
function renderAuctionControls(){
  ...
  if(ff){ff.style.display=canControlSpeed?'inline-flex':'none';ff.textContent=auctionSpeed===1?'⏩ FAST FORWARD':`⏩ ${auctionSpeed}× FAST FORWARD`;ff.title='Click to cycle 1× → 2× → 4× → 10×';}
  if(select){
    select.style.display=canControlSpeed?'inline-block':'none';
    select.value=String([1,2,4,10].includes(auctionSpeed)?auctionSpeed:1);
