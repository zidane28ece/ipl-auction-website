...
function render(){
 ...
 const gapLocked=mode==='ai' && lastExpertBidAt && (performance.now()-lastExpertBidAt)<expertBidGapMs();
 const bidLocked=blocked||gapLocked;
 document.getElementById('bids').innerHTML=[.25,.5,1,2,5].map(x=>`<button class="${bidLocked?'bid-disabled':''}" ${bidLocked?'disabled':''} onclick="place(${x})">+₹${x} Cr</button>`).join('');
 if(gapLocked){
  const remaining=Math.max(0,expertBidGapMs()-(performance.now()-lastExpertBidAt));
  setTimeout(()=>{if(cur&&!auctionEnded)render();},remaining+20);
 }
 const turn=document.getElementById('turnHint'); if(turn)turn.innerHTML=blocked?`⛔ <b>${mine.id}</b> made the last bid — wait for a rival.`:gapLocked?`⏳ <b>Bid window opening…</b>`:`🎯 <b>${mine.id}</b> can bid now`;
...
function home(){clearInterval(timer);clearTimeout(botTimer);
